...
function hoverInfoActivate(context) {
    ...
    const hoverProvider = vscode.languages.registerHoverProvider('marlin', {
        provideHover(document, position, token) {
            ...
            //This will be the same exclusive fore other hover elements that file name doesn't match the G/M code hovered.
            //For example, the G2/G3 page contains information on both G2 and G3
            //Other commands will be processed as if G4 is the same as G04 G004 and G0004 and so on.
            //The same goes for M commands.
            ...
            //M860, M861, M862, M863, M864, M865, M866, M867, M868, M869    
                ...
                return getMarkdownFromFile(gcodeFile);
            //T1 T2    
            }else if (word == 'T1' ||  word == 'T01' || word == 'T001' || word == 'T2' || word == 'T02' || word == 'T002'){
                const gcodeFile = path.join(__dirname, 'MarlinDocumentation-master', '_gcode', 'T1-T2.md');
                return getMarkdownFromFile(gcodeFile);
            }
            else {
